...
def main():
    ...
    if os.path.exists(git_commit_file):
        with open(git_commit_file) as f:
            git_commit = f.read().strip()[:12]
    else:
        import subprocess

        try:
            git_commit = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=os.path.dirname(__file__), stderr=subprocess.DEVNULL).decode().strip()[:12]
        except Exception:
            pass
    if git_commit:
