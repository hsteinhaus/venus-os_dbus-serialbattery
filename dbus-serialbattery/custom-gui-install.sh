...
# GUIv2 plugins API available
guiV2PluginApiAvailable=0
if (( $venusVersionNumber >= $(versionStringToNumber "v3.80~21") )); then
    guiV2PluginApiAvailable=1
fi


# ---------- GUIv2 PLUGIN INSTALLATION ----------

# GUIv2 plugin API available since v3.80~21
if (( $guiV2PluginApiAvailable == 1 )); then

    # copy new JSON if changed or not present
    if ! cmp -s "/data/apps/dbus-serialbattery/qml/plugin/v3.80_21.json" "/data/apps/enabled/dbus-serialbattery/gui-v2/dbus-serialbattery.json"
    then
        echo ""
        echo "Copying JSON file for GUIv2 plugin..."
        mkdir -p "/data/apps/enabled/dbus-serialbattery/gui-v2/"
        cp "/data/apps/dbus-serialbattery/qml/plugin/v3.80_21.json" "/data/apps/enabled/dbus-serialbattery/gui-v2/dbus-serialbattery.json"
        echo "done."
    fi
    ...
    # check if "/opt/victronenergy/gui-v2" is in /data/apps/overlay-fs/overlay-fs.conf
    # If yes remove it, since this is the only clean way to remove GUIv2 custom build
    # GUIv1 will be added in the next step if needed
    if grep -q "/opt/victronenergy/gui-v2" /data/apps/overlay-fs/overlay-fs.conf; then
        echo ""
        echo "Stopping GUI to remove old overlay..."
        # stop gui
        svc -d $servicePath
        # sleep 1 sec
        sleep 1
...
# ---------- GUI & GUIv2 CUSTOM BUILD INSTALLATION ----------
...
# -------------------------- GUIv2 --------------------------
# Do not install custom GUIv2 build if GUIv2 plugin API is available
if (( $guiV2PluginApiAvailable == 0 )); then
    ...
    echo "GUIv2 plugin API is available, no need to install custom GUIv2 build."
fi
