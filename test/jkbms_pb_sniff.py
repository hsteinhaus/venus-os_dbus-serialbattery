...
if TEST in ("all", "modbus"):
    ...
    def build_fc03(addr, start_reg, reg_count):
        msg = bytes([addr, 0x03, (start_reg >> 8) & 0xFF, start_reg & 0xFF, (reg_count >> 8) & 0xFF, reg_count & 0xFF])
        return msg + modbus_crc(msg)
        ...
        for label, cmd in tests:
            ...
            if has_fc03 and not has_55aa:
                ...
                if len(all_bytes) >= 3 and (all_bytes[1] & 0x80):
                    fc = all_bytes[1] & 0x7F
                    err = all_bytes[2]
                    err_names = {1: "illegal function", 2: "illegal address", 3: "illegal data", 4: "CRC error"}
...
if TEST in ("all", "fc03map"):
    ...
    with serial.Serial(PORT, baudrate=BAUD, timeout=0.1) as ser:
        ...
        for base_name, base_addr in [("status 0x1200", 0x1200), ("settings 0x1000", 0x1000), ("about 0x1400", 0x1400)]:
